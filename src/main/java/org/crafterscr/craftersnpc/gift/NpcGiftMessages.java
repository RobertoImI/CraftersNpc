...
public class NpcGiftMessages {
    public static final List<String> TYPES = List.of("favorite", "liked", "disliked", "unknown", "cooldown", "reward", "noReward");
    private static final Map<String, String> DEFAULTS = Map.of(
            "favorite", "¡Wow! ¡Esto me encanta!",
            "liked", "¡Muchas gracias! Esto me gusta mucho.",
            "disliked", "No... la verdad no me gusta eso.",
            "unknown", "No necesito eso ahora.",
    ...
    public boolean remove(String type, int oneBasedIndex) {
        List<String> list = messages.get(type);
        if (list == null || oneBasedIndex < 1 || oneBasedIndex > list.size()) return false;
        list.remove(oneBasedIndex - 1); return true;
    }
    public void clear(String type) { messages.remove(type); }
    public String random(String type, RandomSource random) {
        List<String> list = messages.get(type);
        if (list == null || list.isEmpty()) return DEFAULTS.getOrDefault(type, "Gracias.");
    ...
    public CompoundTag save() {
        ...
        for (String type : TYPES) {
            ...
            for (String msg : messages.getOrDefault(type, List.of())) list.add(StringTag.valueOf(msg));
            if (!list.isEmpty()) tag.put(type, list);
        }
        return tag;
    }
    public static NpcGiftMessages load(CompoundTag tag) {
        NpcGiftMessages result = new NpcGiftMessages();
        for (String type : TYPES) {
            ListTag list = tag.getList(type, Tag.TAG_STRING);
            for (Tag value : list) result.add(type, value.getAsString());
        }
        return result;
    }
}
